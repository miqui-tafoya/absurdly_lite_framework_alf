...
7. **Create Route Files**: In general cases you will need a Model, a Controller and a View for each new route registered, nontheless in some particular situations you will only need the Controller and optionally its Model, for example for pseudoroutes that do not need a view in order to execute methods. In any case, you can either choose to create the corresponding files manually, or you can run the ./build command on your console from the root directory of the project. Take in mind this script was designed for Unix-like OS like Linux or Mac, and you can run it simply by opening a terminal, navigating to the root of your project and running `./build`, but if you are on Windows you can try running `php .\script.php` on Powershell or `php script.php` on CMD. The script has a help manual, accessible through `./build --help`. The most basic command you will want to run, for a complete MVC static GET scenario of a route called /example, would be something like this: `.build --filename example --get --view`. This will create the corresponding files for Controler, Model and View in order for the /example route to work right away. You can try with the other commands to create more complex routes like dynamic routes, or static routes that take string queries, or even routes that don't need a View, in this last case you would just omit the --view parameter.